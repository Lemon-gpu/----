...
def thresholding_image(image) -> cv2.Mat: # 二值化
    ...
    image = cv2.GaussianBlur(image, (5, 5), 0, 0, cv2.BORDER_DEFAULT)
...
def get_top_via_ratio(ratio_list: np.ndarray, threshold: np.float32 = 0.95) -> np.ndarray: # 根据阈值获取前面的元素的像素索引，白色是255，也就是说越高越要切割
    ...
    sorted_index = np.argsort(ratio_list[:, 0])[::-1] # 按照第一列排序，反向排序
...
def vertical_cutout(image: cv2.Mat) -> list: # 垂直切割，原理就是求列的白色像素个数，然后根据比例切割图像啥的
    ...
    ratio_list: np.ndarray = np.dot(image.transpose(), normal_distribution_discretization) / np.dot(image.transpose(), normal_distribution_discretization).max() # 求和的比例，然后根据比例切割图像
    pixel_indices: np.ndarray = get_top_via_ratio(ratio_list, 0.90) # 根据阈值获取前面的元素的像素索引
    return image_split(image, pixel_indices, 1)


def horizenal_cutout(image: cv2.Mat) -> list: # 水平切割，原理就是求行的白色像素个数，然后根据比例切割图像啥的
    ...
    ratio_list = np.dot(image, normal_distribution_discretization) / np.dot(image, normal_distribution_discretization).max() # 求和的比例，然后根据比例切割图像
    pixel_indices: np.ndarray = get_top_via_ratio(ratio_list, 0.90) # 根据经验阈值获取前面的元素的像素索引
...
def test():
    ...
    image = vertical_cutout(horizenal_cutout(image)[0])
    for i in image:
        cv2.imshow("image", i)
        cv2.waitKey(0)

def pipeline(image: cv2.Mat, show_image: bool = False) -> list:
    image = thresholding_image(image)
    image = vertical_cutout(horizenal_cutout(image)[0])
    if show_image:
        for i in image:
